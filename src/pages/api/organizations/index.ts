...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.follow?.length > 0) {
      const create_follow = body.follow;
      body.follow = {
        create: create_follow,
      };
    } else {
      delete body.follow;
    }
    if (body?.stock_idea?.length > 0) {
      const create_stock_idea = body.stock_idea;
      body.stock_idea = {
        create: create_stock_idea,
      };
    } else {
      delete body.stock_idea;
    }
